...
import 'bloc/my_course_detail_events.dart';

class MyCourseDetailController {
  final BuildContext context;

  MyCourseDetailController({required this.context});
  ...
  void init() async {
    ...
    print("id in course detail controller: ${args["courseId"]}");
    asyncLoadCourseData(args["courseId"]);
    asyncLoadSectionData(args["courseId"]);
    ...
    try {
      ...
      if (result != null) {
        if (context.mounted) {
          ...
          context.read<MyCourseDetailBlocs>().add(TriggerMyCourseDetail(result));
        } else {
  ...
  asyncLoadSectionData(int? id) async {
    ...
    try {
      ...
      if (result.sections != null) {
        if (context.mounted) {
          context.read<MyCourseDetailBlocs>().add(
  ...
  //   if(result.code==200){
  //     if(context.mounted){
  //       context.read<MyCourseDetailBloc>().add(TriggerLessonList(result.data!));
